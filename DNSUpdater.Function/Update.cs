...
public class Update
{
        ...
        [HttpTrigger(AuthorizationLevel.Anonymous, "get", Route = null)] HttpRequest req)
    {
        ...
        this.logger.LogDebug($"request details:\nhostname: {hostname}\nip: {ip}\ntoken: {token}\nagent: {agent}\nsystem: {system}");

        token = token.Replace("Basic ", "");
        if (!tokenList.Contains(token)) {
            this.logger.LogWarning($"Unauthorized request. Provided token {token}");
            return new UnauthorizedResult(); //UnauthorizedObjectResult(UpdateStatus.badauth.ToString());
        }
    ...
    {
        return authSection.GetChildren().Select(entry => $"{entry["user"]}:{entry["secret"]}").Select(Base64Encode).ToList();
    }
